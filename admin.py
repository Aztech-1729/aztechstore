...
def deposits_keyboard(
      ...
      rows: list[list[InlineKeyboardButton]] = [
          [
              InlineKeyboardButton(f"🟡 {to_smallcaps('Pending')}", callback_data="admin:deposits:pending:0"),
              InlineKeyboardButton(f"🟢 {to_smallcaps('Confirmed')}", callback_data="admin:deposits:approved:0"),
              InlineKeyboardButton(f"📋 {to_smallcaps('All')}", callback_data="admin:deposits:all:0"),
          ]
      ]
      ...
      if has_prev:
          nav.append(InlineKeyboardButton(f"⬅️ {to_smallcaps('Prev')}", callback_data=f"admin:deposits:{filter_key}:{page-1}"))
      if has_next:
          nav.append(InlineKeyboardButton(f"{to_smallcaps('Next')} ➡️", callback_data=f"admin:deposits:{filter_key}:{page+1}"))
      if nav:
          rows.append(nav)

      rows.append([InlineKeyboardButton(f"⬅️ {to_smallcaps('Back')}", callback_data="admin:menu")])
      return kb(rows)

from config import ADMIN_USER_IDS, TELEGRAM_API_ID, TELEGRAM_API_HASH
...
def admin_menu_keyboard() -> InlineKeyboardMarkup:
    ...
    return kb(
        [
            [
                InlineKeyboardButton(f"➕ {to_smallcaps('Add Account')}", callback_data="admin:addaccount"),
                InlineKeyboardButton(f"👤 {to_smallcaps('Credits')}", callback_data="admin:credits"),
                InlineKeyboardButton(f"📦 {to_smallcaps('Accounts')}", callback_data="admin:accounts"),
            ],
            [
                InlineKeyboardButton(f"💳 {to_smallcaps('Deposits')}", callback_data="admin:deposits"),
                InlineKeyboardButton(f"💰 {to_smallcaps('Active Credits')}", callback_data="admin:activecredits:0"),
            ],
            [
                InlineKeyboardButton(f"🏷️ {to_smallcaps('Bulk Discount')}", callback_data="admin:bulkdiscount"),
            ],
            [
                InlineKeyboardButton(f"🚫 {to_smallcaps('Ban System')}", callback_data="admin:banmenu"),
                InlineKeyboardButton(f"📊 {to_smallcaps('Stats')}", callback_data="admin:stats"),
...
def accounts_menu_keyboard() -> InlineKeyboardMarkup:
    return kb(
        [
            [
                InlineKeyboardButton(f"✅ {to_smallcaps('Available Accounts')}", callback_data="admin:accounts:available:0"),
                InlineKeyboardButton(f"💸 {to_smallcaps('Sold Accounts')}", callback_data="admin:accounts:sold:0"),
            ],
            [InlineKeyboardButton(f"⬅️ {to_smallcaps('Back')}", callback_data="admin:menu")],
        ]
    )
...
async def handle_admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, state: Dict[int, Dict[str, Any]]) -> bool:
    ...
    if data == "admin:menu":
        ...
        await safe_edit(
            ...
            f"<b>╚══════════════════╝</b>\n\n"
            f"<i>🛠️ {to_smallcaps('Choose an option below')}</i>",
    ...
    if data == "admin:addaccount":
        ...
        await query.message.reply_text(
            f"<b>➕ {to_smallcaps('Add Account')}</b>\n\n"
            f"<i>{to_smallcaps('Send phone number with + sign')}</i>\n"
            f"<b>{to_smallcaps('Example')}:</b> <code>+923001234567</code>",
            parse_mode=ParseMode.HTML,
    ...
    if data in {"admin:addaccount:cc:us", "admin:addaccount:cc:ca"}:
        ...
        state[uid] = st
        await query.message.reply_text(
            f"<b>📅 {to_smallcaps('Account Year')}</b>\n\n"
            f"<i>{to_smallcaps('Send year (example: 2023) or type skip')}</i>",
            parse_mode=ParseMode.HTML,
        )
        return True
    ...
    if data == "admin:credits":
        ...
        await safe_edit(
            query.message,
            f"<b>╔══════════════════╗</b>\n"
            f"<b>║  {to_smallcaps('CREDITS MANAGER')}  ║</b>\n"
            f"<b>╚══════════════════╝</b>\n\n"
            f"<i>👤 {to_smallcaps('Choose an action below')}</i>",
            parse_mode=ParseMode.HTML,
            reply_markup=kb(
                [
                    [
    ...
    if data in {"admin:credits:add", "admin:credits:remove"}:
        ...
        await safe_edit(
            query.message,
            f"<b>👤 {to_smallcaps('Credits')} ({to_smallcaps(mode)})</b>\n\n"
            f"<i>{to_smallcaps('Send in one line:')}</i>\n"
            f"<code>&lt;user_id&gt; &lt;amount&gt;</code>\n\n"
            f"<i>{to_smallcaps('Or press Cancel')}</i>",
            parse_mode=ParseMode.HTML,
            reply_markup=kb([[InlineKeyboardButton("⬅️ Back", callback_data="admin:credits")]]),
        )
        # show bottom cancel keyboard too
        await query.message.reply_text(
            f"<i>❌ {to_smallcaps('Press Cancel to stop')}</i>",
            parse_mode=ParseMode.HTML,
            reply_markup=cancel_reply_kb(),
        )
        return True
    ...
    if data == "admin:accounts":
        ...
        await safe_edit(
            query.message, 
            f"<b>╔══════════════════╗</b>\n"
            f"<b>║    {to_smallcaps('ACCOUNTS')}    ║</b>\n"
            f"<b>╚══════════════════╝</b>\n\n"
            f"<b>📦 {to_smallcaps('Choose View')}</b>",
    ...
    if data == "admin:bulkdiscount":
        ...
        text = (
            f"<b>╔══════════════════╗</b>\n"
            f"<b>║ {to_smallcaps('BULK DISCOUNT')} ║</b>\n"
            f"<b>╚══════════════════╝</b>\n\n"
            f"<b>🏷️ {to_smallcaps('Status')}</b>: {status_txt}\n"
            f"<b>📉 {to_smallcaps('Discount')}</b>: <b>{percent}%</b>\n\n"
            f"<i>✅ {to_smallcaps('Applies to ALL available accounts')}</i>\n"
            f"<i>💡 {to_smallcaps('Use Set % to change discount amount')}</i>"
        )
        await safe_edit(
            query.message,
            text,
            ...
            reply_markup=kb(
                [
                    [InlineKeyboardButton(f"✏️ {to_smallcaps('Set %')}", callback_data="admin:bulkdiscount:set")],
                    [
                        InlineKeyboardButton(
                            f"⛔ {to_smallcaps('Turn OFF')}" if enabled else f"✅ {to_smallcaps('Turn ON')}",
    ...
    if data == "admin:bulkdiscount:on":
        ...
        text2 = (
            ...
            f"<b>║ {to_smallcaps('BULK DISCOUNT')} ║</b>\n"
            f"<b>╚══════════════════╝</b>\n\n"
            f"<b>🏷️ {to_smallcaps('Status')}</b>: {status_txt2}\n"
            f"<b>📉 {to_smallcaps('Discount')}</b>: <b>{percent2}%</b>\n\n"
            f"<i>✅ {to_smallcaps('Applies to ALL available accounts')}</i>\n"
            f"<i>💡 {to_smallcaps('Use Set % to change discount amount')}</i>"
        )
        await safe_edit(
            ...
            reply_markup=kb(
                [
                    ...
                    [
                        InlineKeyboardButton(
                            f"⛔ {to_smallcaps('Turn OFF')}" if enabled2 else f"✅ {to_smallcaps('Turn ON')}",
    ...
    if data == "admin:bulkdiscount:set":
        ...
        await query.message.reply_text(
            f"<b>🏷️ {to_smallcaps('Bulk Price Discount')}</b>\n\n"
            f"<i>{to_smallcaps('Send discount percent (0-95)')}</i>\n"
            f"<b>{to_smallcaps('Example')}:</b> <code>20</code>\n\n"
            f"<i>❌ {to_smallcaps('Type Cancel to stop')}</i>",
            parse_mode=ParseMode.HTML,
            reply_markup=cancel_reply_kb(),
        )
        return True

    # admin:qrs removed (UPI only, single QR)
    ...
    if data.startswith("admin:banlist:"):
        ...
        await safe_edit(query.message, "\n".join(lines), parse_mode=ParseMode.HTML, reply_markup=kb(nav))
        return True
    ...
    if data in {"admin:banmenu:ban", "admin:banmenu:unban"}:
        ...
        await query.message.reply_text(
            f"<b>🚫 {to_smallcaps('Ban System')} ({to_smallcaps(mode)})</b>\n\n"
            f"<i>{to_smallcaps('Send user ID:')}</i>\n"
            f"<b>{to_smallcaps('Example')}:</b> <code>6670166083</code>\n\n"
            f"<i>❌ {to_smallcaps('Type Cancel to stop')}</i>",
            parse_mode=ParseMode.HTML,
            reply_markup=cancel_reply_kb(),
        )
    ...
    if data == "admin:stats":
        ...
        text = (
            f"<b>╔══════════════════╗</b>\n"
            f"<b>║   {to_smallcaps('STATISTICS')}   ║</b>\n"
            f"<b>╚══════════════════╝</b>\n\n"
            f"<b>📊 {to_smallcaps('Overview')}</b>\n"
            f"├ 👥 {to_smallcaps('Users')}: <b>{total_users}</b>\n"
            f"├ 📦 {to_smallcaps('Accounts')}: <b>{total_accounts}</b>\n"
            f"├ ✅ {to_smallcaps('Available')}: <b>{available}</b>\n"
            f"└ 💸 {to_smallcaps('Sold')}: <b>{sold}</b>"
        )

        await safe_edit(query.message, text, parse_mode=ParseMode.HTML, reply_markup=kb([[InlineKeyboardButton("⬅️ Back", callback_data="admin:menu")]]))
        return True
    ...
    if data.startswith("admin:account:view:"):
        ...
        text = (
            ...
            f"<b>╚══════════════════╝</b>\n\n"
            f"<b>🆔 {to_smallcaps('ID')}:</b> <code>{acc_id}</code>\n"
            f"<b>📞 {to_smallcaps('Phone')}:</b> <code>{emoji} +{acc.get('phone')}</code>\n"
            f"<b>🌍 {to_smallcaps('Country')}:</b> <b>{country}</b>\n"
            f"<b>📅 {to_smallcaps('Year')}:</b> <b>{year if year is not None else '-'}</b>\n"
            f"<b>📌 {to_smallcaps('Status')}:</b> <b>{status}</b>\n"
            + (f"<b>👤 {to_smallcaps('Sold to')}:</b> {sold_to_line}\n" if sold_to_line else "")
            + f"<b>💰 {to_smallcaps('Price')}:</b> <b>{price if price is not None else 'default'}</b>\n"
            + f"<b>🔒 {to_smallcaps('2FA')}:</b> <b>{'✅ set' if twofa else '❌ not set'}</b>"
        )
        await safe_edit(query.message, text, reply_markup=account_detail_keyboard(acc_id), parse_mode=ParseMode.HTML)
        return True
    ...
    if data.startswith("admin:account:delete:"):
        ...
        await safe_edit(
            query.message,
            f"<b>⚠️ {to_smallcaps('Delete this account?')}</b>\n\n"
            f"<i>{to_smallcaps('This cannot be undone')}</i>",
    ...
    if data.startswith("admin:account:delete_confirm:"):
        ...
        await safe_edit(
            query.message, 
            f"<b>✅ {to_smallcaps('Deleted')}</b>" if ok else f"<i>❌ {to_smallcaps('Account not found')}</i>", 
            reply_markup=None, 
            parse_mode=ParseMode.HTML
        )
        return True
    ...
    if data.startswith("admin:account:edit:"):
        ...
        await query.message.reply_text(
            f"<b>✏️ {to_smallcaps('Edit Account')}</b>\n\n"
            f"<i>{to_smallcaps('Type which field to edit:')}</i>\n"
            f"<code>country</code> | <code>emoji</code> | <code>year</code> | <code>twofa</code> | <code>price</code>\n\n"
            f"<i>❌ {to_smallcaps('Or type cancel to stop')}</i>",
            parse_mode=ParseMode.HTML,
        )
        return True
    ...
    if data.startswith("admin:dep:view:"):
        ...
        if sc and sc.get("file_id"):
            ...
            await safe_edit(query.message, caption + "\n\n❌ Screenshot not stored.", parse_mode=ParseMode.HTML, reply_markup=action_markup)
        return True
    ...
    if data.startswith("admin:dep:setcredits:"):
        ...
        await query.message.reply_text(
            f"<b>💰 {to_smallcaps('Set Credits for Crypto Payment')}</b>\n\n"
            f"<i>{to_smallcaps('How many credits to add?')}</i>\n"
            f"<b>{to_smallcaps('Example')}:</b> <code>1 USDT = 70 credits → send 70</code>",
            parse_mode=ParseMode.HTML,
        )
        return True
...
async def handle_admin_text(
    ...
    if flow == "admin_bulkdiscount":
        if step == "percent":
            if not text.isdigit():
                await update.message.reply_text(
                    f"<i>⚠️ {to_smallcaps('Send a number between 0 and 95 (example: 20)')}</i>",
                    parse_mode=ParseMode.HTML,
                )
                return True
            p = int(text)
            if p < 0 or p > 95:
                await update.message.reply_text(
                    f"<i>⚠️ {to_smallcaps('Percent must be 0-95. Send again:')}</i>",
                    parse_mode=ParseMode.HTML,
                )
                return True
            ...
            try:
                ...
                state.pop(uid, None)
                await update.message.reply_text(
                    f"<b>❌ {to_smallcaps('Failed to apply discount')}:</b> <code>{e}</code>",
                    parse_mode=ParseMode.HTML,
                    reply_markup=main_reply_menu(True),
                )
                return True
            ...
            st2 = await repo.get_bulk_discount()
            await update.message.reply_text(
                f"<b>✅ {to_smallcaps('Bulk Discount Updated!')}</b>\n\n"
                f"<b>🏷️ {to_smallcaps('Status')}:</b> {'ON ✅' if st2.get('enabled') else 'OFF ❌'}\n"
                f"<b>📉 {to_smallcaps('Discount')}:</b> <b>{int(st2.get('percent', 0) or 0)}%</b>",
                parse_mode=ParseMode.HTML,
                reply_markup=main_reply_menu(True),
            )
            return True
    ...
    if flow == "admin_add_account":
        ...
        if step == "phone":
            ...
            if not phone_e164.startswith("+"):
                await update.message.reply_text(
                    f"<i>⚠️ {to_smallcaps('Phone must start with + sign. Send again:')}</i>",
                    parse_mode=ParseMode.HTML,
                )
                return True
            ...
            if needs_choice:
                ...
                state[uid] = st
                await update.message.reply_text(
                    f"<b>🌍 {to_smallcaps('Detected +1 Number')}</b>\n<i>{to_smallcaps('Choose country:')}</i>",
                    parse_mode=ParseMode.HTML,
            ...
            await update.message.reply_text(
                f"<b>📅 {to_smallcaps('Account Year')}</b>\n\n"
                f"<i>{to_smallcaps('Send year (example: 2023), type premium, or skip')}</i>",
                parse_mode=ParseMode.HTML,
            )
            return True
        ...
        if step == "year":
            ...
            if t == "skip":
                ...
                st["step"] = "price"
                await update.message.reply_text(
                    f"<b>💰 {to_smallcaps('Account Price')}</b>\n\n"
                    f"<i>{to_smallcaps('Send price in credits (example: 75)')}</i>",
                    parse_mode=ParseMode.HTML,
                )
                return True
            ...
            if t == "premium":
                ...
                await update.message.reply_text(
                    f"<b>⭐ {to_smallcaps('Premium Selected')}</b>\n\n"
                    f"<i>{to_smallcaps('Send premium months (example: 1, 3, 6, 12)')}</i>",
                    parse_mode=ParseMode.HTML,
                )
                return True
            ...
            if not t.isdigit() or len(t) != 4:
                await update.message.reply_text(
                    f"<i>⚠️ {to_smallcaps('Year must be 4 digits like 2023, or type premium, or skip')}</i>",
                    parse_mode=ParseMode.HTML,
                )
                return True
            st["year"] = int(t)
        ...
        if step == "premium_months":
            if not text.isdigit() or int(text) <= 0:
                await update.message.reply_text(
                    f"<i>⚠️ {to_smallcaps('Send premium months as a number (e.g. 1, 3, 6, 12)')}</i>",
                    parse_mode=ParseMode.HTML,
                )
                return True
        ...
        if step == "price":
            if not text.isdigit():
                await update.message.reply_text(
                    f"<i>⚠️ {to_smallcaps('Price must be numeric. Send again:')}</i>",
                    parse_mode=ParseMode.HTML,
                )
                return True
            st["price"] = int(text)
            ...
            st["step"] = "send_code"
            await update.message.reply_text(
                f"<b>📱 {to_smallcaps('Sending Login Code')}</b>\n\n"
                f"<i>⏳ {to_smallcaps('Sending Telegram login code to this phone...')}</i>",
                parse_mode=ParseMode.HTML,
            )
            await account_manager.admin_begin_login(uid, st["api_id"], st["api_hash"], st["phone_e164"])
            await update.message.reply_text(
                f"<b>🔐 {to_smallcaps('Enter OTP Code')}</b>\n\n"
                f"<i>{to_smallcaps('Send the OTP code received on this number.')}</i>\n"
                f"<i>{to_smallcaps('If 2FA is enabled, I will ask for password next.')}</i>",
                parse_mode=ParseMode.HTML,
            )
            return True
        ...
        if step == "send_code":
            ...
            if status == "need_password":
                st["step"] = "tg_password"
                await update.message.reply_text(
                    f"<b>🔒 {to_smallcaps('2FA Password Required')}</b>\n\n"
                    f"<i>{to_smallcaps('Send your Telegram 2FA password:')}</i>",
                    parse_mode=ParseMode.HTML,
                )
                return True
            ...
            state.pop(uid, None)
            await update.message.reply_text(
                f"<b>✅ {to_smallcaps('Account Saved!')}</b>\n\n"
                f"<i>{to_smallcaps('Account added to stock successfully.')}</i>",
                parse_mode=ParseMode.HTML,
                reply_markup=main_reply_menu(True),
            )
            return True
        ...
        if step == "tg_password":
            ...
            if status in {"invalid_password"} or status != "ok" or not doc:
                ...
                st["step"] = "tg_password"
                await update.message.reply_text(
                    f"<b>❌ {to_smallcaps('Wrong 2FA password.')}</b>\n\n"
                    f"<i>{to_smallcaps('Send again or press Cancel.')}</i>",
                    parse_mode=ParseMode.HTML,
                    reply_markup=cancel_reply_kb(),
                )
                return True
    ...
    if flow == "admin_edit_account":
        ...
        if step == "field":
            ...
            if field not in {"country", "emoji", "year", "twofa", "price"}:
                await update.message.reply_text(
                    f"<b>✏️ {to_smallcaps('Edit Account')}</b>\n\n"
                    f"<i>{to_smallcaps('Choose field to edit:')}</i>\n"
                    f"<code>country</code> | <code>emoji</code> | <code>year</code> | <code>twofa</code> | <code>price</code>\n\n"
                    f"<i>❌ {to_smallcaps('Or type cancel')}</i>",
                    parse_mode=ParseMode.HTML,
                )
                return True
            st["field"] = field
            st["step"] = "value"
            await update.message.reply_text(
                f"<b>✏️ {to_smallcaps('New Value')}</b>\n\n"
                f"<i>{to_smallcaps('Send new value, or type skip to clear')}</i>",
                parse_mode=ParseMode.HTML,
            )
            return True
        ...
        if step == "value":
            ...
            if val_raw.lower() == "skip":
                ...
                if field == "country":
                    ...
                    if not val_raw.isdigit() or len(val_raw) != 4:
                        await update.message.reply_text(
                            f"<i>⚠️ {to_smallcaps('Year must be 4 digits (example: 2023)')}</i>",
                            parse_mode=ParseMode.HTML,
                        )
                        return True
                    ...
                    if not val_raw.isdigit():
                        await update.message.reply_text(
                            f"<i>⚠️ {to_smallcaps('Price must be numeric')}</i>",
                            parse_mode=ParseMode.HTML,
                        )
                        return True
            ...
            state.pop(uid, None)
            await update.message.reply_text(
                f"<b>✅ {to_smallcaps('Updated successfully!')}</b>" if ok else f"<i>⚠️ {to_smallcaps('No changes saved.')}</i>",
                parse_mode=ParseMode.HTML,
            )
            return True
    ...
    if flow == "admin_dep_setcredits":
        if step == "credits":
            if not text.isdigit() or int(text) <= 0:
                await update.message.reply_text(
                    f"<i>⚠️ {to_smallcaps('Send numeric credits (example: 70)')}</i>",
                    parse_mode=ParseMode.HTML,
                )
                return True
            ...
            if not dep:
                state.pop(uid, None)
                await update.message.reply_text(
                    f"<b>❌ {to_smallcaps('Deposit not found or already processed.')}</b>",
                    parse_mode=ParseMode.HTML,
                )
                return True
            ...
            state.pop(uid, None)
            await update.message.reply_text(
                f"<b>✅ {to_smallcaps('Approved and credits added!')}</b>",
                parse_mode=ParseMode.HTML,
            )
            try:
                udoc = await repo.db.users.find_one({"user_id": int(dep["user_id"])})
            ...
            if len(parts) != 2 or not parts[0].isdigit() or not parts[1].lstrip('-').isdigit():
                ...
                if chat_id and msg_id:
                    await context.bot.edit_message_text(
                        chat_id=chat_id,
                        message_id=msg_id,
                        text=f"<b>❌ {to_smallcaps('Invalid format.')}</b>\n<i>{to_smallcaps('Use:')}</i> <code>&lt;user_id&gt; &lt;amount&gt;</code>",
                        parse_mode=ParseMode.HTML,
                        reply_markup=kb([[InlineKeyboardButton("⬅️ Back", callback_data="admin:credits")]]),
                    )
                return True
    ...
    if flow == "admin_ban":
        if step == "input":
            if not text.isdigit():
                await update.message.reply_text(
                    f"<i>⚠️ {to_smallcaps('Send numeric user ID:')}</i>",
                    parse_mode=ParseMode.HTML,
                )
                return True
            target = int(text)
            ...
            if mode == "ban":
                ...
                state.pop(uid, None)
                await update.message.reply_text(
                    f"<b>✅ {to_smallcaps('Banned user')}: <code>{target}</code></b>",
                    parse_mode=ParseMode.HTML,
                    reply_markup=main_reply_menu(True),
                )
                return True
                ...
                state.pop(uid, None)
                await update.message.reply_text(
                    f"<b>✅ {to_smallcaps('Unbanned user')}: <code>{target}</code></b>" if ok else f"<i>⚠️ {to_smallcaps('User was not banned')}: <code>{target}</code></i>",
                    parse_mode=ParseMode.HTML,
                    reply_markup=main_reply_menu(True),
                )
                return True
    ...
    if flow == "admin_credits":
        if step == "user_id":
            ...
            st["step"] = "mode"
            await update.message.reply_text(
                f"<b>👤 {to_smallcaps('Credits Mode')}</b>\n\n"
                f"<i>{to_smallcaps('Type add to add credits or set to replace credits:')}</i>",
                parse_mode=ParseMode.HTML,
            )
            return True
        ...
        if step == "mode":
            ...
            if m not in {"add", "set"}:
                await update.message.reply_text(
                    f"<i>⚠️ {to_smallcaps('Type add or set:')}</i>",
                    parse_mode=ParseMode.HTML,
                )
                return True
            st["mode"] = m
            ...
            await update.message.reply_text(
                ...
                f"<i>{to_smallcaps('Send amount as a number:')}</i>",
                parse_mode=ParseMode.HTML,
            )
            return True
        ...
        if step == "amount":
            if not text.lstrip("-").isdigit():
                await update.message.reply_text(
                    f"<i>⚠️ {to_smallcaps('Send numeric amount:')}</i>",
                    parse_mode=ParseMode.HTML,
                )
                return True
            amt = int(text)
            ...
            state.pop(uid, None)
            await update.message.reply_text(
                f"<b>✅ {to_smallcaps('Credits Updated!')}</b>\n\n"
                f"├ 👤 {to_smallcaps('User')}: <code>{target}</code>\n"
                f"└ 💰 {to_smallcaps('Credits')}: <b>{user.get('credits', 0)}</b>",
                parse_mode=ParseMode.HTML,
                reply_markup=main_reply_menu(True),
            )
            return True
