...
class ReturnDocument:
    """Replaces pymongo.ReturnDocument — motor's find_one_and_update accepts True/False directly."""
    AFTER = True
    BEFORE = False
...
class Repo:
    ...
    async def buy_account_filtered(
        ...
        q = dict(base_q)
        q["price"] = {"$ne": None, "$lte": int(credits)}
    ...
    async def buy_account_by_id(
        ...
        # annotate returned account for UI
        account["_original_price"] = original_price
        account["_final_price"] = charge
        account["_discount_used"] = False
